...
interface CreateTaskModalProps {
  isOpen: boolean;
  onClose: () => void;
  onCreate: (taskData: Omit<Task, 'id' | 'createdAt'>) => Task;
  projects: Project[];
  skills: Skill[];
}
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/75 backdrop-blur-sm animate-in fade-in duration-150">
      ...
      >
        <div className="flex items-center justify-between pb-3 border-b border-white/[0.06]">
          <div>
            <h2 className="text-sm font-semibold text-neutral-100">Create New Task</h2>
            <p className="text-[11px] text-neutral-400 mt-0.5">
              Add a fully configured action item to your execution pipeline.
            </p>
          </div>
          <button
            onClick={onClose}
        ...
        <form onSubmit={handleSubmit} className="space-y-3.5 text-xs">
          ...
          <div className="flex items-center justify-end gap-2 pt-3 border-t border-white/[0.06]">
            ...
            <button
              type="submit"
              className="flex items-center gap-1.5 px-4 py-1.5 rounded-lg bg-neutral-100 hover:bg-white text-neutral-950 font-medium transition-colors"
            >
